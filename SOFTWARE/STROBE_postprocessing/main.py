...
BINNED_SIPS_MODE = False
TIME_BIN = 1
TIME_CUM_BIN_SIPS = 1
TOTAL_NUM_SIPS = False
PREF_INDEX = False
IMPOSSIBLE_PREF_INDEX = -999999999999999
...
pref_index_base = "pref_index"

# base main
if __name__ == '__main__':
    raw_filename = raw_input("Please enter the path to your cap data file "
        "(output from the STROBE system):\n")
    
    # assume drag and drop filename
    filename = string.replace(raw_filename, "\\", "/")
    ...
    cum_sips_query = raw_input("Calculate and save cumulative sips vs "
        "time? (Y/N):\n")
    CUMULATIVE_SIPS_MODE = strobelib.parse_YN_query(cum_sips_query)
    if CUMULATIVE_SIPS_MODE:
        cum_bin_sips_query = raw_input("Bin cumulative number of sips vs time? (Y/N):\n")
        CUMULATIVE_BIN_SIPS_MODE = strobelib.parse_YN_query(cum_bin_sips_query)

        if CUMULATIVE_BIN_SIPS_MODE:
            time_cum_bin_sips_query = raw_input("Enter the ideal time bin as an integer "
            "(amount of time per bin divided by 10 ms):\n")
            TIME_CUM_BIN_SIPS = strobelib.parse_numeric_query(time_cum_bin_sips_query, TIME_CUM_BIN_SIPS)
    
    duration_sips_query = raw_input("Calculate and save sip durations? (Y/N):\n")
    DURATION_SIPS_MODE = strobelib.parse_YN_query(duration_sips_query)
    
    binned_sips_query = raw_input("Calculate and save binned number of sips? "
    ...
    if BINNED_SIPS_MODE:
        time_bin_query = raw_input("Enter the ideal time bin as an integer "
            "(amount of time per bin divided by 10 ms):\n")
        TIME_BIN = strobelib.parse_numeric_query(time_bin_query, TIME_BIN)
        
    total_num_sips_query = raw_input("Calculate and save total "
        "number of sips? (Y/N):\n")
    ...
    pref_index_query = raw_input("Calculate and save the preference index? (Y/N):\n")
    PREF_INDEX = strobelib.parse_YN_query(pref_index_query)

    if PREF_INDEX:
       pref_index = []

    data_frame = strobelib.read_dataframe(filename, START_TIME, STOP_TIME);
        ...
        if not BINNED_SIPS_MODE and TOTAL_NUM_SIPS:
                ...
                if PREF_INDEX:
                    if not (total_num_sips_1 == 0 and total_num_sips_1 == 0):
                        preference = float(total_num_sips_1 - total_num_sips_2)/(total_num_sips_1 + total_num_sips_2)
                    else:
